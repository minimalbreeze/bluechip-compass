/* ============================================================================
   ...
   이 사이트는 빌드 도구도 서버도 없는 정적 사이트다. "오늘의 시황"을 파일에
   적어 넣으면 배포한 다음 날부터 틀린 정보가 되고, 초보자는 그 낡은 문장을
   최신 정보로 믿는다. 정세는 하루 만에 뒤집히는데 파일은 그대로 있는 게
   가장 위험하다.

   그래서 시황을 "적어주는" 대신 **판정 규칙**을 넣었다.
     · 사용자가 5개 다이얼을 오늘 값으로 맞춘다 (각 30초, 확인 링크 제공)
     · 규칙 엔진이 국면을 판정하고 → 배분 조정 + 이번 달 행동을 내놓는다
   그러면 파일이 낡아도 결과는 항상 오늘 기준이 된다. 덤으로 초보자가
   "무엇을 봐야 하는지"를 배운다 — 사이트가 사라져도 남는 능력이다.

   DEFAULTS의 `asOf`는 마지막으로 사람이 확인한 날짜다. UI가 이 날짜와 오늘을
   비교해 며칠 지났는지 크게 표시하고, 오래됐으면 경고를 띄운다.
   기본값을 갱신할 때는 반드시 `asOf`도 같이 고칠 것.
   ========================================================================== */
...
window.BCMarket = (function () {
  ...
  var DIALS = [
    ...
    },
    {
      ...
      why: '미장 투자자에게 환율은 두 번째 종목이다. 주가가 그대로여도 환율로 손익이 갈린다.',
      where: [
  ...
  function actions(s, marketKey) {
    ...
    if (marketKey === 'us') {
      if (s.fx === 'weak') {
        out.push({ icon: '💱', t: '환전을 나눠서 하거나 원화 상품으로 대체',
          d: '고환율에 전액 환전하면 나중에 환차손이 난다. 국내상장 미국ETF(원화로 매수)를 섞는 것도 방법이다.' });
      } else if (s.fx === 'strong') {
        out.push({ icon: '🛒', t: '달러가 쌀 때 환전을 조금 더 해둔다',
          d: '지금 사지 않더라도 달러로 바꿔 초단기 국채(SGOV 등)에 두면 실탄이 된다.' });
      }
  ...
  var DEFAULTS = {
    ...
    note: '이 기본값은 사이트를 만든 시점에 사람이 채워둔 출발점일 뿐입니다. ' +
          '<b>정세는 하루 만에 뒤집힙니다.</b> 위 링크로 직접 확인하고 다이얼을 오늘 값으로 바꾸세요 — 그 순간부터 결과는 오늘 기준이 됩니다.'
  };
  ...
  return {
    ...
    /* 다이얼 상태 → 배분 조정치(%p) 합산. 극단으로 튀지 않게 상·하한을 둔다. */
    tilt: function (state) {
      var cash = 0, sat = 0;
